...
{
    public partial class frmBaoCaoCongLamThem : Form
    {
        public frmBaoCaoCongLamThem()
        {
            InitializeComponent();
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            int j = 0;
            DateTime date;
            MessageBox.Show(records.Rows.Count.ToString());
            foreach (DataRow row in records.Rows)
            {
                ...
                for (int i = 1; i <= 31; ++i)
                {
                    ...
                    {
                        double t = Double.Parse(tmpTable.Rows[0]["LamThemGio"].ToString());
            ...
            dtgCongLamThem.DataSource = newTable;
            ...
            for (int i = 1; i <= 31; ++i)
            {
                if (i < DateTime.DaysInMonth(Int32.Parse(dtpThang.Value.ToString("yyyy")), Int32.Parse(dtpThang.Value.ToString("MM"))))
                {
                    ...
                    {
                        dtgCongLamThem.Columns[i + 1].DefaultCellStyle.BackColor = Color.FromArgb(212, 211, 209);
                    }
                }
            }
        }

        private void frmBaoCaoCongLamThem_Load(object sender, EventArgs e)
        {
            dtgCongLamThem.Columns.Clear();
        }
    }
